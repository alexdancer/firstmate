...
root=Path.cwd(); lab=root/'.cmux-validation-r3'; env=os.environ.copy()
env.update(PATH=str(lab/'bin')+':/Applications/cmux.app/Contents/Resources/bin:'+env['PATH'],TMPDIR=str(lab/'tmp'),FM_TEST_SKIP_ORPHAN_REAP='1',GIT_CONFIG_GLOBAL='/dev/null',GIT_CONFIG_NOSYSTEM='1')
for mode in ['send-failure','precommit','publication','dispatch']:
    ...
    subprocess.run([str(root/'bin/fm-lab-home.sh'),'create',str(home)],check=True,stdout=subprocess.DEVNULL)
    ...
    ev=Path('/Users/alex/.no-mistakes/evidence/01M3D2JD5ZBG8VV8D72A3EZKJQ/round3')/mode; ev.mkdir(exist_ok=True)
    for f in ['spawn.log','at-publication.busy','dispatch-attempts','send-fault']:
        if (case/f).exists(): shutil.copyfile(case/f,ev/f)
    (ev/'result.json').write_text(json.dumps(summary,indent=2));print(json.dumps(summary),flush=True)
