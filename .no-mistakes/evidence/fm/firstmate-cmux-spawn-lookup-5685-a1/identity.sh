...
set -eu
ROOT=$PWD
export FM_HOME="$ROOT/.cmux-live-phase/identity-home"
mkdir -p "$FM_HOME"
export PATH="$ROOT/.cmux-live-phase/bin:/Applications/cmux.app/Contents/Resources/bin:$PATH"
...
ids=$(fm_backend_cmux_create_task "$label" "$ROOT/.cmux-live-phase")
read -r ws sf <<< "$ids"
target="$ws:$sf"
printf 'created %s\n' "$target"
...
new=$(command cmux new-surface --workspace "$ws" --working-directory "$ROOT/.cmux-live-phase" --command '/bin/bash --noprofile --norc' --focus false --json --id-format uuids)
printf '%s\n' "$new"
...
stale=$target
target="$ws:$new_sf"
marker="$ROOT/.cmux-live-phase/wrong-endpoint-marker"
