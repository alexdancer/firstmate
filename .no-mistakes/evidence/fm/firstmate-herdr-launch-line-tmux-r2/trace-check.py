...
root=pathlib.Path.cwd(); scratch=root/'.local-test-tmp/trace-retry'; scratch.mkdir(parents=True,exist_ok=True)
...
env.update(TMPDIR=str(root/'.local-test-tmp'), FM_HERDR_LAB_STATE_DIR=str(scratch/'labs'),GIT_CONFIG_GLOBAL='/dev/null',GIT_CONFIG_NOSYSTEM='1',FM_GATE_REFUSE_BYPASS='1',FM_TRACE_CONTEXT='on')
...
def case(label,spawn,blocked=False,long=False):
    ...
    (home/'config'/'lavish-axi-host').write_text('http://127.0.0.1:4387/\n')
    payload='LONG-END-'+('abcd0123'*900) if long else 'normal'
    worker=d/'worker.py';worker.write_text('import os,json,sys,time\nfrom pathlib import Path\nPath('+repr(str(marker))+').write_text(json.dumps({"task":os.getenv("FM_TASK_ID"),"gotmp":os.getenv("GOTMPDIR"),"compact":os.getenv("COMPACT_ADVISER_DISABLE"),"trace":os.getenv("TRACEPARENT"),"host":os.getenv("LAVISH_AXI_HOST"),"payload":sys.argv[1]}))\nprint("WORKER_STARTED: environment received; candidate preserved",flush=True)\ntime.sleep(300)\n')
    ...
    run(['bash','-c','. bin/fm-trace-context-lib.sh; fm_trace_context_session_start "$FM_HOME/config" "$FM_HOME/state/.trace-context-effective"'],e=launch_env)
    ...
    if blocked:
        ...
        assert 'traceparent=' not in (home/'state'/f'{ident}.meta').read_text()
        note('Trace metadata not published after setup failure')
        note('Blocked setup refused without launching; shell response: '+screen)
    else:
        assert p.returncode==0 and started
        data=json.loads(marker.read_text());trace=data.pop('trace');host=data.pop('host');meta=(home/'state'/f'{ident}.meta').read_text();assert f'traceparent={trace}\n' in meta;assert host=='http://127.0.0.1:4387/';note(f'TRACEPARENT worker={trace}; metadata identical; LAVISH_AXI_HOST={host}');assert data=={'task':ident,'gotmp':f'/tmp/fm-{ident}/gotmp','compact':'1','payload':payload}
...
try:
    ...
    case('trace-ready',root/'bin/fm-spawn.sh')
    case('trace-blocked',root/'bin/fm-spawn.sh',blocked=True)
finally:
    ...
    (ev/'trace-launch-transcript.txt').write_text('\n\n'.join(transcript)+'\n')
